...
def index_to_action(index):
    if index==0:
    ...
    elif index>=len(contain_hop)+1 and index<=1+len(contain_hop)+len(contain_hop)*(len(contain_hop)-1)/2-1:
        ...
        index_temp=int(index-(1+len(contain_hop)+len(contain_hop)*(len(contain_hop)-1)/2-1))
        k=0
        i=int((len(contain_hop)-k-1)*(len(contain_hop)-k-2)/2)
        while 1:
            if index_temp<=i:
                aa1=k
                index_temp_temp=index_temp
                ii=len(contain_hop)-k-2
                kk=1
                while ii>0:
                    if index_temp_temp<=ii:
                        bb2=kk
                        cc2=index_temp_temp+bb2
                        break
                    else:
                        index_temp_temp=index_temp_temp-ii
                        ii=ii-1
                        kk=kk+1
                bb1=aa1+bb2
                cc1=aa1+cc2
                ...
                k=k+1
                i=int((len(contain_hop)-k-1)*(len(contain_hop)-k-2)/2)
        a=machine_name_to_index(contain_hop[aa1])
        b=machine_name_to_index(contain_hop[bb1])
        c=machine_name_to_index(contain_hop[int(cc1)])
        return [a,b,c]
...
def action_to_index(action):
    ...
    if action==[]:
        ...
        aaa=contain_hop.index(machine_index_to_name(action[0]))
        bbb=contain_hop.index(machine_index_to_name(action[1]))
        ccc=contain_hop.index(machine_index_to_name(action[2]))
        ...
        c=list_abc[2]
        final=0
        for i in range(len(contain_hop)):
            if not list_abc[0]==i:
                final+= int((len(contain_hop)-i-1)*(len(contain_hop)-2-i)/2)
            else:
                break
        final+=int((b-a-1)*(len(contain_hop)-a-1+len(contain_hop)-b-1)/2+c-b)
        return int(len(contain_hop)+len(contain_hop)*(len(contain_hop)-1)/2)+final
